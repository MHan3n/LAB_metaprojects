...
```{r}
data.frame(TEXT = data_factors$Q10_1, CODE = data_numeric$Q10_1) %>% distinct() %>% arrange(CODE)
data.frame(TEXT = data_factors$Q10_2, CODE = data_numeric$Q10_2) %>% distinct() %>% arrange(CODE)
...
```{r}
data_numeric %>% select(starts_with("Q1_")) %>%
