...
```{r}
subset(items, key %in% grep("^Q16_",key,value=TRUE) )%>%
  select(value) %>% gsub(pattern = "For each of the statements below, please indicate whether or not the statement is characteristic of you or of what you believe. - ",replacement = "")
```
...
```{r}
subset(items, key %in% grep("^Q17_",key,value=TRUE) )%>%
  select(value) %>% gsub(pattern = "Read each of the following statements and decide how much you agree with each according to your beliefs and experiences. - ",replacement = "")
```
